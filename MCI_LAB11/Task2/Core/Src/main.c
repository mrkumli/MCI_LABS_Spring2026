...
int main(void)
{
  ...
  // --> NEW: Start the Motor PWM channels! (CH2 = PA1, CH3 = PA2)
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOC, RIGHT_DIR1_Pin|RIGHT_DIR2_Pin|LEFT_DIR1_Pin|LEFT_DIR2_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : RIGHT_DIR1_Pin RIGHT_DIR2_Pin LEFT_DIR1_Pin LEFT_DIR2_Pin */
  GPIO_InitStruct.Pin = RIGHT_DIR1_Pin|RIGHT_DIR2_Pin|LEFT_DIR1_Pin|LEFT_DIR2_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim->Instance == TIM4) {
      ...
      // Send the calculated speed to the hardware!
...
void Set_Motor_Speeds(float pid_value) {
    ...
    // 1. Determine Direction based on PID sign
    if (pid_value > 0) { 
        // Tilting Forward -> Drive Forward
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_0, GPIO_PIN_SET);   // Right Motor Forward
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_RESET);
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_2, GPIO_PIN_SET);   // Left Motor Forward
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_3, GPIO_PIN_RESET);
        final_pwm = pid_value;
    } 
    else if (pid_value < 0) { 
        // Tilting Backward -> Drive Backward
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_0, GPIO_PIN_RESET); // Right Motor Backward
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, GPIO_PIN_SET);
        HAL_GPIO_WritePin(GPIOC, GPIO_PIN_2, GPIO_PIN_RESET); // Left Motor Backward
    ...
    // 2. Clamp to Maximum Speed (999)
    if (final_pwm > MAX_PWM) {
        final_pwm = MAX_PWM;
    }

    // 3. Send speed to the hardware timer registers
    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, (uint32_t)final_pwm); // Right Motor (PA1)
    __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, (uint32_t)final_pwm); // Left Motor (PA2)
}
